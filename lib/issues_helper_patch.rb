...
module IssuesHelperPatch
  ...
  module InstanceMethods
    def render_descendants_tree_with_partial_amounts(issue)
      if issue.is_bill_bundle?
        s = '<form><table class="list issues">'
        issue_list(issue.descendants.visible.preload(:status, :priority, :tracker).sort_by(&:lft)) do |child, level|
          child_bill_invoice_amount = child.custom_values.find_by_custom_field_id(Setting.plugin_redmine_emergya_adjustments['bill_invoice_custom_field'])
          ...
          s << content_tag('tr',
                 ...
                 content_tag('td', ((child_bill_invoice_amount.present? and child_bill_invoice_amount.value.present?) ? child_bill_invoice_amount.value.to_f.round(2).to_s + '€' : '')),
                 :class => css)
        end
        s << '</table></form>'
        s.html_safe
